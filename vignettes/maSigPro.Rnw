...
\\
{\bf maSigPro} follows a two steps regression strategy to find genes with significant temporal expression changes and significant differences between experimental groups.
...
The procedure adjusts this global model by the least squared technique to identify differentially expressed genes and selects significant genes applying a false discovery rate control procedure.
